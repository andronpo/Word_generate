...
std::string WordPhrase::stringify() {
			...
			(StringifyableArray(new std::vector<Stringifyable*>{
				...
				new XMLContentTag("w:r",
					new StringifyableArray(new std::vector<Stringifyable*>{
						new XMLContentTag("w:rPr",
							&params),
						new XMLContentTag("w:t", new std::vector<XMLTagParameter>{ XMLTagParameter("xml:space", "preserve") }, new XMLContent(this->text))}))
				}
	).stringify());
}


WordPhrase::WordPhrase(std::string text, bool bold, bool italic, bool underlined) {
	this->text = text;
	this->bold = bold;
	this->italic = italic;
